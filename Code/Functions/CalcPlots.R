...
SnoutAnglePlot <- function(df) {
  df |>
    ggplot(aes(
      ...
      y = snoutangle.deg,
      group = ID,
      color = as.character(Speed.group)
    )) +
    theme_bw() +
...
SlowSnoutVelocityPlot <- function(df, meansnoutvel) {
  df |>
    group_by(ID) |>
    mutate(tmax = t.sec[which.max(snoutangle.degps)], newT = t.sec - tmax) |>
    ...
    geom_path(
      data = meansnoutvel,
      aes(x = newT, y = meansnoutvel.degps),
      inherit.aes = FALSE,
      size = .75,
      color = 'deepskyblue'
    )
...
MeanMoiMeanSnoutAngVelPlot <- function(
  df,
  meanSnoutAng.lm,
  meanFastSnoutAng.lm,
  meanSlowSnoutAng.lm
  ...
  df |>
    ...
    ylim(c(20, 400)) +
    geom_abline(
      slope = coef(meanFastSnoutAng.lm)[["meanpercMoI"]],
      intercept = coef(meanFastSnoutAng.lm)[["(Intercept)"]],
      size = 1,
      color = 'firebrick'
    ) +
    geom_abline(
      slope = coef(meanSlowSnoutAng.lm)[["meanpercMoI"]],
...
TorqueMoITimingPlot <- function(df) {
  df <- df |>
    group_by(ID) |>
    reframe(
      tmaxT = t.sec[which.max(torque.gcm2ps2)],
...
TotalPecStrokesPlot <- function(df, totalpecstrokes.lm) {
  df |>
    ...
    theme_classic() +
    geom_abline(
      slope = coef(totalpecstrokes.lm)[["totaltime"]],
...
PecStrokesPerTimePlot <- function(df, pecstrokes) {
  df |>
    ...
    geom_smooth(
      data = pecstrokes,
      aes(x = totaltime, y = StrokesPerTime.sps),
      method = "lm",
      formula = y ~ I(1 / x),
      inherit.aes = FALSE,
      color = 'black',
...
BackingDataPlot <- function(df, backingdata.lm) {
  df |>
    ...
    theme_classic() +
    geom_abline(
      slope = coef(backingdata.lm)[["totaltime"]],
...
PecFinAmplitudePlot <- function(df) {
  df |>
    ggplot(
      aes(y = pecangle.deg, x = side, color = Speed.group),
      color = Speed.group
    ) +
...
linearmomentumplot <- function(df, speedgroup, speedcolor, y1, y2) {
  df |>
    filter(Speed.group == speedgroup) |>
    ggplot(
      aes(y = meanlinearmomentum.gnormcmps, x = Time),
      color = speedcolor
    ) +
    geom_swarm(dotsize = .5, alpha = .15, color = speedcolor) +
    theme_classic() +
