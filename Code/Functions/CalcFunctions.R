...
calcCentroid <- function(df) {
  df |>
    group_by(ID, frame) |>
    mutate(
      centroidx = (sum(xs.cm * mass.g) / sum(mass.g)),
...
calcAngChange <- function(df) {
  df |>
    ...
    mutate(
      bleh = case_when(
        min(tempsnoutangle.deg, na.rm = TRUE) < -100 ~ 'BLEH',
        min(tempsnoutangle.deg, na.rm = TRUE) > -100 ~ 'ARGH'
      )
    ) |>
    rowwise() |>
    mutate(
      snoutangle.deg = case_when(
        bleh == 'BLEH' ~ -tempsnoutangle.deg,
...
calcAngVel <- function(df, dt) {
  df |>
    ...
    reframe(
      snoutangle.rad = snoutangle.rad[1],
      snoutangle.deg = snoutangle.deg[1],
      bleh = bleh
    ) |>
    group_by(ID) |>
    mutate(
      dang = lead(snoutangle.rad) - lag(snoutangle.rad),
...
calcAngChangeTail <- function(df) {
  df |>
    ...
    mutate(
      bleh = case_when(
        min(temptailangle.deg, na.rm = TRUE) < -100 ~ 'BLEH',
        min(temptailangle.deg, na.rm = TRUE) > -100 ~ 'ARGH'
      )
    ) |>
    rowwise() |>
    mutate(
      tailangle.deg = case_when(
        bleh == 'BLEH' ~ -temptailangle.deg,
...
calcAngVelTail <- function(df, dt) {
  df |>
    group_by(ID, frame, t.sec) |>
    summarize(
      tailangle.rad = tailangle.rad[1],
      tailangle.deg = tailangle.deg[1]
    ) |>
    ungroup() |>
    group_by(ID) |>
    mutate(
      dang = lead(tailangle.rad) - lag(tailangle.rad),
...
calcMoI <- function(df) {
  ...
  dataMoI <- dataMOIcentroid %>%
    ...
    mutate(
      MoI.gcm2 = sum(
        (sqrt((xs.cm - centroidx)^2 + (ys.cm - centroidy)^2)^2 * mass.g)
      )
    )
...
calcMaxMoI <- function(df) {
  ...
  dataMOIcentroid %>%
    ...
    group_by(Individual) |>
    summarise(Individual = first(Individual), MaxMoI.gcm2 = first(MaxMoI.gcm2))
}
...
runSnoutAngButter <- function(df, bf) {
  df |>
    group_by(ID) |>
    mutate(
      snoutangle.deg = EndEffect(snoutangle.deg, bf),
...
runTailAngButter <- function(df, bf) {
  df |>
    group_by(ID) |>
    mutate(
      tailangle.rad = EndEffect(tailangle.rad, bf),
...
calcTorque <- function(df) {
  df |>
    ...
    group_by(ID, bodypart) |>
    mutate(
      segangvel.radps = nderiv(segang.rad, dt),
      segangaccel.radps2 = nderiv(segangvel.radps, dt)
    ) |>
    calcCentroid() |>
    ungroup() |>
    mutate(
      r.cm = sqrt((ys.cm - centroidy)^2 + (xs.cm - centroidx)^2),
      r2.cm2 = r.cm^2
    ) |>
    group_by(ID, bodypart) |>
...
calcMeanMoIMeanSnoutVel <- function(df) {
  df |>
    ...
    group_by(ID) |>
    mutate(
      percMoI = MoI.gcm2 / (max(MoI.gcm2, na.rm = TRUE)),
