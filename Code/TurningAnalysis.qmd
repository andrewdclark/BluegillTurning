...
```{r}
angvel_timediff_pred <- predictions(
  angvel_timediff_model,
  newdata = datagrid(Individual = NA,
                     TimeDiff.sec = seq(-1, 0.9, by = 0.1)),
  re.form=NA)

angvel_timediff_plot <-
  angvel_timediff_pred |> 
...
maindata |> 
  ...
  filter(near(t.sec, tsnoutmax, tol = 2/60) |
           near(t.sec, ttailmax, tol = 2/60)) |> 
  ggplot(aes(xs.cm, ys.cm)) +
